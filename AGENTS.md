...
Canonical context for AI assistants working in this repository. Use this as the single starting point for project rules, structure, and workflow.

## What This Is

Personal AI agent orchestrator. Python 3.11+. Privacy-first, local-model-first, self-improving with human approval gates. Orchestrates specialized agents (code, research, file operations), runs primarily on local models (Ollama), sandboxes all file operations to a workspace, and self-improves only with human approval. Pre-implementation phase — architecture and tooling are set, application code is not.
...
Optional `.env`: `WORKSPACE_PATH`, `DATABASE_PATH`, `OLLAMA_HOST`, `LOG_LEVEL`, `ANTHROPIC_API_KEY`. Never commit `.env` or credentials.

## Code Rules — Non-Negotiable

- **Python 3.11+.** No constructs unavailable in 3.11.
- **Type hints on everything.** Every function signature and return type. `disallow_untyped_defs = true` in mypy. No shortcuts.
- **Line length: 100.** Black enforces this.
- **Google-style docstrings** on all public functions and classes.
- **Ruff must pass clean.** Rules: E, W, F, I, B, C4, UP, ARG, SIM. `E501` ignored (Black handles line length).
- **MyPy must pass strict.** `warn_return_any`, `strict_equality`, `no_implicit_optional` — all enabled. Fix type errors; do not suppress with `# type: ignore` unless unavoidable and documented.
...
- `tests/` — mirrors `agent/` with `unit/` and `integration/`
...
- Path traversal is a critical vulnerability. Validate all paths.
...
- [docs/spec.md](docs/spec.md) — Technical spec: data models, API, security model, database schema
- [docs/roadmap.md](docs/roadmap.md) — Implementation phases
- [docs/dev-guide.md](docs/dev-guide.md) — Development setup and workflow
